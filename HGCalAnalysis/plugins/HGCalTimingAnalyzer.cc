...
//#include "RecoNtuples/HGCalTimingAnalyzer/interface/AEvent.h"
  ...
  for (std::vector<CaloParticle>::const_iterator it_caloPart = caloParticles.begin(); it_caloPart != caloParticles.end(); ++it_caloPart){
	...
	for(reco::HGCalMultiCluster::component_iterator it = multiClusters[i].begin();
	  ...
	  for(unsigned int j = 0; j < hf.size(); j++){
	    ...
	    if(hit->time() > 0){
	      ...
	      float dRwrtGen = dsGenRecHit(etaGen, phiGen, rhL, rhX, rhY);
	      float dRwrtMCl = dsGenRecHit(etaMC, phiMC, rhL, rhX, rhY);
	  ...
	  float Radius_2DClGen = dsGenRecoObj(etaGen, phiGen, z2DCl, x2DCl, y2DCl);
	  float Radius_2DClMCl = dsGenRecoObj(etaMC, phiMC, z2DCl, x2DCl, y2DCl);
    ...
    */
	...
	if(found){
	  ...
	  float dR_RhGen = sqrt( pow(rhX - axX, 2) + pow(rhY - axY, 2) + pow(rhZ - axZ, 2) );
	  //	  if(int(dR_RhlGen) < 5) sumMtcRHEnergy_vsDR_wrtGen[int(dR_RhGen)] += rhEnergy*it_haf->second;
	  ...
	  //	  if(int(Radius_RhGen) < 5) sumMtcRHEnergy_vsRadius_wrtGen[int(Radius_RhGen)] += rhEnergy*it_haf->second;	  

	  /*
	  h_rHGen_dR->Fill(Radius_RhGen);
	  h_rHGen_dDist->Fill(dR_RhGen);
	  h2_rHGen_dR_vsEta->Fill(std::abs(etaGen), Radius_RhGen);
	  h2_rHGen_dDist_vsEta->Fill(std::abs(etaGen), dR_RhGen);
	  tp_rHGen_dDist_vsEta->Fill(std::abs(etaGen), dR_RhGen);
	  h2_Energia_vsEta->Fill(std::abs(etaGen), it_caloPart->energy());
	  h2_Pt_vsEta->Fill(std::abs(etaGen), it_caloPart->pt());
	  h2_rHGen_dR_vsEnergia->Fill(it_caloPart->energy(), Radius_RhGen);
	  h2_rHGen_dR_vsPt->Fill(it_caloPart->pt(), Radius_RhGen);

	  tp2_Energy_vs_Eta_dR->Fill(std::abs(etaGen), it_caloPart->energy(), Radius_RhGen);
	  tp2_Energy_vs_Eta_dDist->Fill(std::abs(etaGen), it_caloPart->energy(), dR_RhGen);

	  h2_rHGen_dRcosh_vsEta->Fill(std::abs(etaGen), Radius_RhGen/cosh(etaGen));
	  h2_rHGen_dRcoshP_vsEta->Fill(std::abs(etaGen), Radius_RhGen*cosh(etaGen));
	  */
	  ...
	  if(rhTime > -1.){
	    int timesOfRadius = int(Radius_RhGen );
	    ...
	    // h_rhGen_RadiusDdeta_Eta[etaBin]->Fill(Radius_RhGen / std::abs(rhEta - etaGen));
	    // h_rhGen_RadiusPdeta_Eta[etaBin]->Fill(Radius_RhGen * std::abs(rhEta - etaGen));
	    ...
	    h2_allRH_TimesVsDRaxis->Fill(dR_RhGen, rhTime);
	    tp_allRH_TimesVsDRaxis->Fill(dR_RhGen, rhTime);
	    ...
	    h2_allRH_TimesVsRadiusaxis->Fill(Radius_RhGen, rhTime);
	    tp_allRH_TimesVsRadiusaxis->Fill(Radius_RhGen, rhTime);
	  }
    ...
    } // sim cluster from calo particle
